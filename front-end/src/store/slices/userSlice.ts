...
interface UserState {
  ...
  adminUserList: PagedResult<User>;
  loading: boolean;
  error: string | null;
}
...
const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        fetchExpertises.fulfilled,
        (state, action: PayloadAction<Expertise[]>) => {
          state.loading = false;
...
export const { clearProfile, updateAvatarSuccess, updateCertificatesSuccess } =
  userSlice.actions;
